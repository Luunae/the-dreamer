...
class QuoteUnroll(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        urls = [url for url in message.content.split() if url.startswith("http")]
        for url in urls:
            if url_is_valid(url) and ("discord" in url) and ("channels" in url):
                message_from_url = await self.get_message_from_url(url)
